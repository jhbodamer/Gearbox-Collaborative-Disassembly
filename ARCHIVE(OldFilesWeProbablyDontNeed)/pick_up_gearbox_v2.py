                    ...
                    if not hasattr(main, 'step3_complete') or main.step3_complete is None:
                        ...
                        if hasattr(main, 'step3_complete') and not hasattr(main, 'step4_complete'):
                            ...
                            try:
                                ...
                                # Calculate tool-frame offset in base frame
                                # Get current TCP transformation matrix
                                T_base_tcp = ur_pose6_to_T(current_pose)
                                
                                # Tool-frame offset vector
                                tool_offset = np.array([-0.05, 0.003, 0.0, 1.0])  # [dx, dy, dz, 1] in tool frame
                                
                                # Transform offset to base frame
                                base_offset = T_base_tcp @ tool_offset
                                
                                # Create target pose with offset applied
                                target_pose_offset = [
                                    base_offset[0],
                                    base_offset[1],
                                    base_offset[2],
                                ...
                                print(f"Target pose (with offset): [{target_pose_offset[0]:.3f}, {target_pose_offset[1]:.3f}, {target_pose_offset[2]:.3f}, {target_pose_offset[3]:.3f}, {target_pose_offset[4]:.3f}, {target_pose_offset[5]:.3f}]")
                                rtde_control.moveL(target_pose_offset, 0.1, 0.1)
                                print("[OK] Tool-frame offset applied")

                                time.sleep(1)

                                # Then move to Z height (260mm)
                                current_pose_after_offset = get_tcp_pose6(rtde)
                                target_pose_step4 = [
                                    current_pose_after_offset[0],
                                    current_pose_after_offset[1],
                                    0.260,
                                    current_pose_after_offset[3],
                                    current_pose_after_offset[4],
                                    current_pose_after_offset[5]
                                ]
                                ...
                                rtde_control.moveL(target_pose_step4, 0.1, 0.1)
                                print("[OK] Z move to 260mm executed")
