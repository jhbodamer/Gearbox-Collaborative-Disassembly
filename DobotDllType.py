...
""" Experimental: Infrared and Color Sensor Ports """

class InfraredPort(IntEnum):
    PORT_GP1 = 0
    PORT_GP2 = 1
    PORT_GP4 = 2
    PORT_GP5 = 3
...
class UART4PeripheralsType(IntEnum):
...
class InfraredCmd(Structure):
    _fields_ = [
        ("port",     c_uint8),   # use InfraredPort.value
        ("isEnable", c_bool),
        ("version",  c_uint8),
    ]
...
def load():
    if platform.system() == "Windows":
        lib = CDLL("./DobotDll.dll", RTLD_GLOBAL)
    elif platform.system() == "Darwin":
        lib = CDLL("libDobotDll.dylib", RTLD_GLOBAL)
    else:
        lib_path = os.path.join(os.path.dirname(__file__), "libDobotDll.so")
        lib = cdll.LoadLibrary(lib_path)

    # 🔒 SAFE FUNCTION SIGNATURES
    lib.GetPose.argtypes = [POINTER(Pose)]
    lib.GetPose.restype = c_int

    lib.GetEndEffectorGripper.argtypes = [POINTER(c_int), POINTER(c_int)]
    lib.GetEndEffectorGripper.restype = c_int

    lib.GetEndEffectorSuctionCup.argtypes = [POINTER(c_int), POINTER(c_int)]
    lib.GetEndEffectorSuctionCup.restype = c_int

    lib.GetColorSensor.argtypes = [POINTER(c_ubyte), POINTER(c_ubyte), POINTER(c_ubyte)]
    lib.GetColorSensor.restype = c_int

    lib.SetInfraredSensor.argtypes = [c_bool, c_uint8]
    lib.SetInfraredSensor.restype = c_int

    lib.GetInfraredSensor.argtypes = [c_uint8, POINTER(c_ubyte)]
...
""" Provided by Dr. Zhou, doesn't currently work """
...
""" ENTIRELY EXPERIMENTAL, NOT TESTED """

def SetInfraredSensor(api, port: InfraredPort, enable: bool, version: int, isQueued: bool):
    cmd = InfraredCmd(port.value, enable, version)
    queued = c_uint64(0)
    for _ in range(10):
        result = api.SetInfraredSensor(byref(cmd), c_bool(isQueued), byref(queued))
        if result == DobotCommunicate.DobotCommunicate_NoError:
            return [queued.value]
        dSleep(5)
    raise RuntimeError("SetInfraredSensor timeout")

def GetInfraredSensor(api, port: InfraredPort):
    val = c_uint8(port.value)
    out = c_uint8(0)
    for _ in range(10):
        result = api.GetInfraredSensor(val, byref(out))
        if result == DobotCommunicate.DobotCommunicate_NoError:
            return out.value
        dSleep(5)
    raise RuntimeError("GetInfraredSensor timeout")
