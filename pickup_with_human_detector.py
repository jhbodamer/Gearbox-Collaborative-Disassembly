...
# ---------- USER CONFIG ----------
# Human detection webcam (adjust index as needed)
HUMAN_DETECTOR_CAM_INDEX = 2
UR_IP = "192.168.1.5"
...
def wait_for_idle_state(work_detector, human_cam):
    """
    Monitor the WorkDetector until it confirms IDLE state.
    Returns True when safe to proceed, False if user cancels.
    """
    print("\n" + "="*60)
    print("Waiting for workspace to be clear...")
    print("="*60 + "\n")
    
    while True:
        ret, frame = human_cam.read()
        if not ret:
            print("[ERROR] Could not read from human detection camera")
            return False
        ...
        # Check work status
        is_working, confidence, debug_info = work_detector.is_person_working(frame)
        
        # Draw status on frame
        frame = work_detector.draw_status(frame, is_working, confidence, debug_info)
        
        # Add instruction overlay
        cv2.putText(frame, "ROBOT WAITING FOR IDLE STATE", 
                   (10, frame.shape[0] - 60),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        
        # Display frame
        cv2.imshow('Human Safety Monitor', frame)
        
        # Check for IDLE state
        if not is_working:
            print("\n" + "="*60)
            print("WORKSPACE CLEAR - IDLE STATE CONFIRMED")
            print(f"  Idle time: {debug_info['idle_time']:.1f}s")
...
def main():
    ...
    print("[1/6] Initializing Human Detection System...")
    work_detector = WorkDetector(idle_threshold=2.0)
    
    human_cam = cv2.VideoCapture(HUMAN_DETECTOR_CAM_INDEX)
    if not human_cam.isOpened():
        print(f"[ERROR] Could not open human detection camera (index {HUMAN_DETECTOR_CAM_INDEX})")
        return
    print("  ✓ Human detection camera ready")
    
    # --- Wait for IDLE state ---
    print("\n[2/6] Checking workspace safety...")
    if not wait_for_idle_state(work_detector, human_cam):
        human_cam.release()
    ...
    # Close human detection window (we're clear to proceed)
    cv2.destroyWindow('Human Safety Monitor')
    print("  ✓ Workspace clear - proceeding with robot operations")
    
    # --- Original initialization continues ---
    print("\n[3/6] Loading camera calibration...")
    ...
    try:
        ...
        while not sequence_complete:
            ...
            if object_detected and rtde_control is not None:
                        ...
                        if hasattr(main, 'step5_complete') and not hasattr(main, 'step6_complete'):
                            ...
                            if time_since_delay >= 0.5:
                                ...
                                try:
                                    ...
                                    print(f"Current TCP: [{current_pose[0]:.3f}, {current_pose[1]:.3f}, {current_pose[2]:.3f}, {current_pose[3]:.3f}, {current_pose[4]:.3f}, {current_pose[5]:.3f}]")
                                    print(f"Target pose: [{target_pose_step6[0]:.3f}, {target_pose_step6[1]:.3f}, {target_pose_step6[2]:.3f}, {target_pose_step6[3]:.3f}, {target_pose_step6[4]:.3f}, {target_pose_step6[5]:.3f}]")
                        ...
                        if hasattr(main, 'step8_complete') and not hasattr(main, 'step9_complete'):
                            ...
                            if time_since_delay >= 0.5:
                                ...
                                try:
                                    ...
                                    current_pose = get_tcp_pose6(rtde)
                                    z_move = z_move + 0.01
                                    target_pose_step9 = [randomx, randomy, current_pose[2], current_pose[3], current_pose[4], current_pose[5]]  # move random xy
                                    print(f"Current TCP: [{current_pose[0]:.3f}, {current_pose[1]:.3f}, {current_pose[2]:.3f}, {current_pose[3]:.3f}, {current_pose[4]:.3f}, {current_pose[5]:.3f}]")
                                    ...
                                    target_joints = [
                                        ...
                                        current_joints[2],  # current theta3
                                        current_joints[3],  # current theta4
                                        current_joints[4],  #current theta5
                                        current_joints[5] + randomj  # current theta6 + random angle
                                    ]
                                    print(f"Target joints: [{np.degrees(target_joints[0]):.1f}, {np.degrees(target_joints[1]):.1f}, {np.degrees(target_joints[2]):.1f}, {np.degrees(target_joints[3]):.1f}, {np.degrees(target_joints[4]):.1f}, {np.degrees(target_joints[5]):.1f}] deg")
                        ...
                        if hasattr(main, 'step9_complete') and not hasattr(main, 'step10_complete'):
                            ...
                            if time_since_delay >= 1.0:
                                ...
                                try:   
                                    ...
                                    # Reset sequence for next run - BUT MARK AS COMPLETE (NO LOOP)
                                    object_detected = False
                                    main.step10_complete = True
                                    
                                    print("\n" + "="*70)
                                    print("PICKUP SEQUENCE COMPLETE")
                                    print("="*70 + "\n")
                                    
                                    # Mark sequence as complete to exit the loop
                                    sequence_complete = True
                                    
                                except Exception as e:
                                        print(f"[ERROR] RESET FAILED {e}")
                                        main.sequence_start_time = None
                                        sequence_complete = True  # Exit even on error
        ...
        work_detector.pose.close()
        cv2.destroyAllWindows()
        print("Stopped RealSense and human detection systems.")
